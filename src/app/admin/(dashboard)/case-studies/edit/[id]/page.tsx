...
export default function EditCaseStudyPage() {
      ...
      try {
        ...
        if (data) {
          setFormData({
            ...
            metric1: data.metrics?.[0] || '',
            metric2: data.metrics?.[1] || '',
            metric3: data.metrics?.[2] || '',
          });
        }
      } catch (err: any) {
        console.error('Error fetching case study details:', err);
        setMessage({ type: 'error', text: err.message || 'Failed to load case study details.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateCaseStudyAction(caseId, {
      ...
      setMessage({ type: 'success', text: 'Case study updated successfully! Redirecting...' });
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-12 w-full">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight leading-none">Edit Case Study</h1>
          <p className="text-sm text-slate-500 mt-2 font-medium">Update client story details and outcomes.</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex items-center gap-2 py-12 text-slate-400">
          ...
          <span className="text-sm font-semibold">Loading case study details...</span>
        </div>
      ) : (

...
export default function EditVideoPage() {
  ...
  useEffect(() => {
    const fetchVideoDetails = async () => {
      ...
      try {
        ...
        setMessage({ type: 'error', text: err.message || 'Failed to load video details.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateVideoAction(videoId, {
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-12 w-full">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight leading-none">Edit Video</h1>
          <p className="text-sm text-slate-500 mt-2 font-medium">Update YouTube video link and configurations.</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex items-center gap-2 py-12 text-slate-400">
          ...
          <span className="text-sm font-semibold">Loading video details...</span>
        </div>
      ) : (
